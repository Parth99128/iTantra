#!/usr/bin/env python3
"""Fetch real offline model packs for iTantra.

A pack contains only the selected language's models plus the shared runtime
assets required for that pack. The Android app installs packs into one shared
private directory and therefore multiple language packs can coexist.
"""
...
def download(url):
 req=urllib.request.Request(url,headers={"User-Agent":"iTantra-model-fetcher/4.0"}); last=None
...
def extract_tgz(data,destination,strip_root=True):
 with tarfile.open(fileobj=io.BytesIO(data),mode="r:bz2") as tar:
  ...
  root=next(iter(roots)) if strip_root and len(roots)==1 else None
...
def write_source(destination,source,kind):
 (destination/"MODEL_SOURCE.json").write_text(json.dumps({"source":source,"runtime":"Vosk" if kind=="stt" else "sherpa-onnx VITS","offline_runtime":True,"pack_kind":kind},indent=2),encoding="utf-8")
...
def build_pack(language,stt,tts):
 shutil.rmtree(ASSETS,ignore_errors=True); ASSETS.mkdir(parents=True,exist_ok=True); fetch_vad(); fetch_stt([stt] if stt!="none" else []); fetch_tts([tts]);
...
def main():
 ...
 stt=a.stt.strip(); tts=a.tts.strip();
 if stt not in STT_PACKS and stt!="none": raise SystemExit("Unsupported STT model pack requested")
 if tts not in TTS_PACKS and tts not in PIPER_TTS_PACKS: raise SystemExit("Unsupported TTS model pack requested")
 ASSETS.mkdir(parents=True,exist_ok=True)
 if not a.skip_vad: fetch_vad()
 fetch_stt([] if stt=="none" else [stt]); fetch_tts([tts])
