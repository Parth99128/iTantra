#!/usr/bin/env python3
"""Fetch real offline model assets for one iTantra language pack."""
...
def download(url):
 req=urllib.request.Request(url,headers={"User-Agent":"iTantra-model-fetcher/5.0"})
 last=None
...
def extract_tgz(data,destination):
 with tarfile.open(fileobj=io.BytesIO(data),mode="r:bz2") as tar:
  members=tar.getmembers(); roots={Path(m.name).parts[0] for m in members if Path(m.name).parts}
  root=next(iter(roots)) if len(roots)==1 else None
  for member in members:
   parts=Path(member.name).parts
   if not parts or (root and parts[0]!=root): continue
   relative=Path(*parts[1:]) if root else Path(*parts)
...
def fetch_vad():
 ...
 target.parent.mkdir(parents=True,exist_ok=True)
 if not target.exists(): target.write_bytes(download(VAD_URL))
def fetch_espeak(destination):
 data_dir=destination/"espeak-ng-data"
 if data_dir.exists(): return
 data_dir.mkdir(parents=True,exist_ok=True)
 extract_tgz(download(ESPEAK_URL),data_dir)
 if not (data_dir/"phontab").exists():
  nested=next((p for p in data_dir.rglob("phontab") if p.is_file()),None)
  if nested:
   src=nested.parent
   for item in src.iterdir(): shutil.move(str(item),data_dir/item.name)
 if not (data_dir/"phontab").exists(): raise RuntimeError("Offline eSpeak-ng phonemizer data missing")
def write_source(destination,source,kind):
 (destination/"MODEL_SOURCE.json").write_text(json.dumps({"source":source,"offline_runtime":True,"pack_kind":kind},indent=2),encoding="utf-8")
def fetch_stt(lang):
 if lang=="none": return
 archive=STT_PACKS[lang]; destination=ASSETS/"vosk"/lang; destination.mkdir(parents=True,exist_ok=True)
 extract_vosk(download(f"{VOSK_BASE}/{archive}"),destination,archive[:-4]); write_source(destination,archive,"stt")
def fetch_sherpa_tts(lang,archive):
 ...
 fetch_espeak(ASSETS/"tts")
 write_source(destination,archive,"tts")
def fetch_piper_tts(lang,pack):
 ...
 onnx.save(model,str(destination/"model.onnx")); fetch_espeak(ASSETS/"tts")
 write_source(destination,f"rhasspy/piper-voices/{pack['onnx']}","tts"); source_model.unlink(missing_ok=True); source_config.unlink(missing_ok=True)
def fetch_tts(lang):
...
def main():
 ...
 if a.stt not in STT_PACKS and a.stt!="none": raise SystemExit("Unsupported STT model pack requested")
 if a.tts not in TTS_PACKS and a.tts not in PIPER_TTS_PACKS: raise SystemExit("Unsupported TTS model pack requested")
 ASSETS.mkdir(parents=True,exist_ok=True)
 if not a.skip_vad: fetch_vad()
 fetch_stt(a.stt); fetch_tts(a.tts)
