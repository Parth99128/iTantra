...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { ITantraTheme { Surface(Modifier.fillMaxSize()) { WalkieTalkieScreen(viewModel, onPickModelPack = { modelPackPicker.launch(arrayOf("application/zip", "application/octet-stream")) }) } } }
    }
}
...
fun WalkieTalkieScreen(viewModel: WalkieTalkieViewModel, onPickModelPack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Text("Offline Neural Transceiver Radio", style = MaterialTheme.typography.bodyMedium)
        Spacer(Modifier.height(12.dp))
        Button(onClick = onPickModelPack) { Text("Install Offline Model Pack") }
        Text("Models are stored locally; internet is never used during inference.", style = MaterialTheme.typography.labelSmall)
        ...
        Box {
            OutlinedButton(onClick = { langMenuExpanded = true }) { Text(state.language.displayName) }
            DropdownMenu(expanded = langMenuExpanded, onDismissRequest = { langMenuExpanded = false }) {
                SupportedLanguage.values().forEach { lang ->
                    DropdownMenuItem(text = { Text(lang.displayName) }, onClick = { viewModel.loadModels(lang); langMenuExpanded = false })
                }
            }
        }
        Spacer(Modifier.height(8.dp))
        ...
        Row {
            ...
            Spacer(Modifier.width(8.dp))
            OutlinedButton(onClick = { }) { Text("Join (Phone B)") }
        }
        Spacer(Modifier.height(32.dp))
        val ptt = Modifier.size(160.dp).pointerInteropFilter { event ->
            when (event.action) {
        ...
        Button(onClick = {}, modifier = ptt, shape = MaterialTheme.shapes.extraLarge, enabled = state.mode == OperatingMode.WALKIE_TALKIE) {
